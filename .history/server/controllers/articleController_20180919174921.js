...
export async function createArticle(ctx) { // 新建文章，需传入title、content、abstract、tags、publish
    ...
    const abstract = ctx.request.body.abstract; // 摘要
    ...
    let createResult = await article.save();
...
export async function uploadArticleImage(ctx) {
    let form = new formidable.IncomingForm();
    let post = {};
    let file = {};
    form.uploadDir = '/server/static/images';
    form
    .on('error', function(err) {
        console.log(err); //各种错误
    })
     //POST 普通数据 不包含文件 field 表单name value 表单value 
    .on('article_pic', function(article_pic, value) { 
        if (form.type == 'multipart/form-data') {  //有文件上传时 enctype="multipart/form-data" 
            if (article_pic in post) { //同名表单 checkbox 返回array 同get处理
                if (util.isArray(post[article_pic]) === false) {
                    post[article_pic] = [post[article_pic]];
                }
                post[article_pic].push(value);
                return;
            }
        }
        post[article_pic] = value;
    })
    .on('file', function(article_pic, file) { //上传文件
        file[article_pic] = file;
    })
    .on('end', function() {
         //解析完毕 做其他work
    });
    form.parse(ctx.request.body); //解析request对象

    console.log(form.parse(ctx.request.body));
