<template>
    <div class="article-pos">
        <article class="article" v-for="(item, index) of article_list" :key="index" @click="goToArticle(item)">
            ...
            <div class="content">
                <p>最初，我只是想整理一份技术清单，理清思路。但是内容越写越多，索性就做成一个教程，放在 Github 上面。这份教程一共四章，可以在两天内学完，适合那些已掌握前端基本技能（HTML、CSS、JS）、想进一步进修、成为 JavaScript 全栈工程师的读者。</p>
    ...
    export default {
        ...
        created() {
            this.getAllArticle().then((res) => {
              if (res.data.success) {
                  this.article_list = res.data.articleArr
              }  
            });
        },
        methods: {
