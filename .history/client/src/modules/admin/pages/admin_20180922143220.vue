<template>
    <div>
        <el-form class="article-form" method="post" enctype="multipart/form-data">
            <el-form-item>
...
<script>
    ...
    import { timeStamp } from '../../../../static/js/utils.js';
    export default {
        data() {
            return {
                title: '',
        ...
        created() {
            ...
            //     }
            // });
            console.log(this.$store.state.tag_list);
        },
        methods: {
            upload(option) {
            ...
            confirmAddTag() {
                ...
                this.$store.dispatch('createTag', send_data).then((res) => {
                    if (res.data.success) {
            ...
            submit() {
                ...
                this.$store.dispatch('createArticle', send_data).then((res) => {
                    if (res.data.success) {
                        this.$message.success('success');
                    } else {
                        this.$message.error(res.data.message);
