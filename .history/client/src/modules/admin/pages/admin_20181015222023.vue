    ...
    <div id="article-edit">
        <el-form inline class="article-form" method="post" enctype="multipart/form-data">
            ...
            <el-form-item label="标签: ">
                <el-select class="el-input-article" placeholder="请选择标签" v-model="tag_sel" multiple>
...
<script>
    ...
    export default {
        ...
        created() {
            let article_id = window.location.href.split('id=')[1];
            if (article_id) {
                this.getArticleById(article_id).then(res => {
                    if (res.data.success) {
                        let data = res.data.article;
                        this.title = data.title;
                        this.tag_list = data.tags;
                        this.markdown_value = data.content;
                        this.article_image = data.image_url;
                        this.abstract = data.abstract;
                        this.publish = data.publish ? '1' : '0';
                    }
                });
            }
            this.getAllTags();
