<template>
    <div>
        <el-form method="post" enctype="multipart/form-data">
            ...
            <el-form-item>
                <el-upload ref="pic_upload" drag action="" :auto-upload="false" :on-change="ossUpload" :file-list="file">
...
<script>
    ...
    export default {
        ...
        methods: {
            async ossUpload(file, file_list) {
                try {
                    ...
                    const suffix = file.name.substr(0, file.name.lastIndexOf("."));
                    ...
                    if (res.res.status === 200) {
                        this.$refs.pic_upload.$img2Url(file.name, 'http://article-images.oss-cn-beijing.aliyuncs.com/' + save_path)
                    } else {
