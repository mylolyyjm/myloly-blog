<template>
    <div>
        <el-form method="post" enctype="multipart/form-data">
            ...
            <el-form-item>
                <el-upload ref="pic_upload" drag action="" :http-request="ossUpload(file)" :file-list="file">
             ...
             ossUpload(file) {
                try {
                    let vm = this;
                     ...
                     this.axios.get('/api/getOssSign').then(res => {
                        this.oss_sign = res.data;
                    })
                    const client = new OSS(this.oss_sign);
                    const suffix = file[0].name.substr(file[0].name.lastIndexOf("."));
                    const file_name = file.name.substr(0, file[0].name.lastIndexOf("."))
                    const time_stamp = this.timeStamp();
                    
                    const save_path = 'article/' + time_stamp + suffix; //  路径+时间戳+后缀名
                    console.log(save_path,file);
                    
                    let res =  client.multipartUpload(save_path, file , (err,res) =>);
                    console.log(res);
